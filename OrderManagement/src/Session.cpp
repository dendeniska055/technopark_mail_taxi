...
void
Session::SendOrderInformation(MailTaxi::ProposeOrderRequest &locationData, std::function<void(beast::error_code)> handler) {
    std::vector<char> message(locationData.ByteSizeLong() + 1);
    message[0] = orderInformationCode;
    locationData.SerializeToArray(message.data() + 1, locationData.ByteSizeLong());
    ws.async_write(net::buffer(message),[handler](beast::error_code ec, std::size_t msgLen){
        handler(ec);
    });
}
